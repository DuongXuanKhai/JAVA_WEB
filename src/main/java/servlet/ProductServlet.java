...
public class ProductServlet extends HttpServlet {// tạo class ProductServlet kế thừa HttpServlet của thư viện svl.api
	private static final long serialVersionUID = 1L;
	//  khởi tạo  biến ProductDAO trong lớp ProductServlet để thực hiện các hoạt động liên quan đến truy cập cơ sở dữ liệu (database).
	...
	public ProductServlet() {
		super();
	// Tạo đối tượng ProductDAO thông qua phương thức getInstance của lớp ProductDAO
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int permission = 0;// biến này được tạo ra để xem thử đã đăng nhập hay chưa
		// kiểm tra xem đã đăng nhập hay chưa, nếu chưa thì nó sẽ trả về err, còn đăng đăng nhập rồi thì nó sẽ...
		if (request.getSession().getAttribute("user") != null) {
			// Đã đăng nhập -> check quyền admin
			UserSession userSession = (UserSession) request.getSession().getAttribute("user");
			// kiểm tra xem có quyền admin hay k 
	...
	// Thêm sản phẩm
	//nó nhận đầu vào là một đối tượng HttpServletRequest chứa các thông tin về yêu cầu của client, 
	//và một đối tượng HttpServletResponse để trả về kết quả phản hồi cho client. 
	//dùng phương thức post để che thông tin trên http req, 
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("application/json");
		response.setCharacterEncoding("UTF-8");
		request.setCharacterEncoding("UTF-8");
	//được sử dụng để lấy một đối tượng PrintWriter từ đối 
	//tượng HttpServletResponse để ghi các kết quả trả về vào phản hồi HTTP.
		...
		// Nó kiểm tra loại nội dung (content type) của một yêu cầu HTTP được gửi đến
 		// và kiểm tra xem nó có phải là "multipart/form-data" hay không.
 		// Nếu nội dung của yêu cầu không phải là "multipart/form-data", đoạn
  		// mã trên sẽ trả về một phản hồi JSON cho client với thông báo lỗi "API chỉ hỗ trợ multipart/form-data".
		// multipart/form-data là 1 loại định dạng dữ liệu được sử dụng để gửi các tệp đính kèm hoặc các loại dữ liệu
		...
		try {
			//Nếu biến "formItems" không phải là null và có kích thước lớn hơn 0, 
			//thì tiếp tục thực hiện các lệnh bên trong khối lệnh.
			...
			if (formItems != null && formItems.size() > 0) {
				...
				for (FileItem item : formItems) {
					if (!item.isFormField()) {
						// File
						// Xử lý extension
						// đoạn code dưới được sử dụng để lấy vị trí của dấu '.' cuối cùng trong chuối tên file
						int index = item.getName().lastIndexOf(".");
						if (index < 0) {
						// nếu không đúng thì hiển thị mes nếu đúng thì nó sẽ tiếp tục xử lý
							out.write(gson.toJson(new JsonResponse(false, "Không hỗ trợ định dạng file này.")));
							return;
						}
						// nhận định dạng file có đuôi .jpg .png .jpeg 
						String spExt = ".jpg .png .jpeg";
						String ext = item.getName().substring(index).toLowerCase();
						// và nếu file nào k có thì thì nó sẽ trả về mess
